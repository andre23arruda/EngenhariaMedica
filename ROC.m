...
%     - AUC: area abaixo da curva
  ...
  nclasse1 = numel(classe1); % numero de padroes da classe 1
  nclasse2 = numel(classe2); % numero de padroes da classe 2

  x = (sortrows([classe1', ones(nclasse1,1); classe2', 2*ones(nclasse2,1)]))';
  % criando um vetor com os valores da caracteristica das duas classes e um vetor que corresponde a 
  % qual classes os valores pertencem e ordenando os valores
  ...
  VPR(i+1) = 0; % adicionando a posicao final como zero
  FPR(i+1) = 0; % adicionando a posicao final como zero

  AUC = abs(sum(diff(FPR).*VPR(1:length(VPR)-1))); % calculando a area abaixo da curva atraves do somatorio das areas de cada retangulo da curva
